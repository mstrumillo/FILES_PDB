...
def get_pdb_name(ali_file):
	alignment_file=open(ali_file,"r").readlines()
	pdb_name_line=alignment_file[-2].split(";")
	a=pdb_name_line[0].lstrip(">").split()[0]
	start=pdb_name_line[1].strip()
	pdb_name=a+"+"+start
	return(pdb_name)
...
#arguments argv[1] - ali file
alignment_file=argv[1]
domain_name=alignment_file.split("_")[0]
pdb_name=get_pdb_name(alignment_file)
...
#############make a dataframe that only contains pdb rows
def pdb_dataframe(pdb_name,letter_alignment, position_alignment, background_df, header):
	rows_with_pdb=position_alignment.index[position_alignment[pdb_name]>0].tolist()
	pdb_positions=position_alignment.ix[rows_with_pdb,pdb_name].tolist()
	rows_with_pdb2 = [x+1 for x in rows_with_pdb]
	pdb_background=background_df.ix[rows_with_pdb2,header]
	hotspots=[0]*len(rows_with_pdb2)
	pdb_background[pdb_name]=pdb_positions
	pdb_background['hotspot']=hotspots
	pdb_background['hotspot_plus']=hotspots
	pdb_background=pdb_background.reset_index()
	pdb_background=pdb_background.rename(columns={"index": "alignment_pos"})
	return(pdb_background)

header=["foreground", "bg_means", "bg_stdev", "pvals","minus_logs", "regs","active"]
background_dataframe.to_csv('%s_pval_pdb_ali'%domain_name,columns=header)
################
pdb_df=pdb_dataframe(pdb_name, letter_alignment, position_alignment, background_dataframe, header)
treshold=6.70e-08
fore_val=2.0

############another column for hotspots assignment
def assign_hotspots(pdb_df, treshold, fore_val):
	rows_that_are_hotspots=pdb_df.index[(pdb_df['pvals']<=treshold) & (pdb_df['foreground']>=fore_val)].tolist()
	print(rows_that_are_hotspots)
	for i in rows_that_are_hotspots:
		print(pdb_df.loc[[i]])
		pdb_df.at[i,'hotspot']+=1
		pdb_df.at[i,'hotspot_plus']+=1
		if i-2>=0:
			pdb_df.at[i-2,'hotspot_plus']+=1
		if i-1>=0:
			pdb_df.at[i-1,'hotspot_plus']+=1
		if i+1<=pdb_df.index[-1]:
			pdb_df.at[i+1,'hotspot_plus']+=1
		if i+2<=pdb_df.index[-1]:
			pdb_df.at[i+2,'hotspot_plus']+=1


assign_hotspots(pdb_df, treshold, fore_val)
header2=["alignment_pos", pdb_name, "foreground", "bg_means", "bg_stdev", "pvals", "minus_logs", "regs", "active", "hotspot_plus"]
pdb_df.to_csv("pval_pdb",columns=header2)
